...
namespace ShopOnline.Areas.Admin.Controllers
{
    ...
    public class CRUDproductController : Controller
    {
        ...
        public ActionResult Index(int? page, string searching)
        {
            ...
            var pageSize = 10;
            var products = db.Products.Where(model => model.productName.Contains(searching) || searching == null).OrderByDescending(model => model.dateCreate)
                .Include(model => model.Member)
                .Include(model => model.ProductCategory)
                .Include(model => model.Size)
        ...
        public ActionResult Create()
        {
            ...
            ViewBag.SizeId = new SelectList(db.Size, "SizeId", "SizeName");
        ...
        public ActionResult Create(Product product, HttpPostedFileBase uploadFile)
        {
            try
            {
                ...
                ViewBag.SizeId = new SelectList(db.Size, "SizeId", "SizeName", product.SizeId);
                return View(product);
            }
            catch (Exception ex)
            {
